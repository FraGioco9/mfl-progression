...
- `site/modules/core-sources/shared-foundations.js` owns universal state, table/view configuration, constants, and DOM bindings; `site/modules/core-sources/shared.js` owns universal navigation, behavior, and synchronous cross-route entry points. Evaluation table rendering, overall controls, render reuse and snapshot hydration belong to `evaluation.js`, loaded by the existing Evaluation route loader. The shared source keeps a 355,000-byte universal ceiling so route-specific behavior cannot silently return to code parsed by every route. Route/domain core sources intentionally have no hard byte ceilings: if they grow, split them when ownership, readability, or route cost warrants it rather than to satisfy an arbitrary number.
