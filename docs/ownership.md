...
- `site/modules/core-sources/shared-foundations.js` owns universal state, table/view configuration, constants, and DOM bindings; `site/modules/core-sources/shared.js` owns universal navigation, behavior, and synchronous cross-route entry points. The canonical core manifest owns their order and validation must consume the complete Shared domain rather than either fragment in isolation. Validators that need Shared or the full canonical core should use `validate-core-sources.mjs` rather than duplicating physical fragment paths; generated-equivalence validators must compare `app-core-runtime.js` against the assembled Shared domain, not one physical fragment. Feature validators whose contract spans both Shared foundations and Shared behavior must likewise read the assembled Shared domain. Evaluation table rendering, overall controls, render reuse and snapshot hydration belong to `evaluation.js`, loaded by the existing Evaluation route loader. The shared source keeps a 355,000-byte universal ceiling so route-specific behavior cannot silently return to code parsed by every route. Route/domain core sources intentionally have no hard byte ceilings: if they grow, split them when ownership, readability, or route cost warrants it rather than to satisfy an arbitrary number.
