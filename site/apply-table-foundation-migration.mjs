...
function write(path, content) {
  writeFileSync(resolve(repoRoot, path), content, "utf8");
}

function replaceOnce(path, before, after) {
  const source = read(path);
  const first = source.indexOf(before);
  if (first < 0) throw new Error(`${path}: expected migration source was not found:\n${before}`);
  if (source.indexOf(before, first + before.length) >= 0) {
    throw new Error(`${path}: migration source is ambiguous:\n${before}`);
  }
...
for (const tempPath of [
  "site/apply-table-foundation-migration.mjs",
  ".github/workflows/apply-table-foundation-migration.yml",
]) {
